...
Enum.each(regioes, fn(regiao) ->
  StatesApi.Repo.insert!(StatesApi.Regiao.changeset(%StatesApi.Regiao{}, regiao))
end)
...
Enum.each(states, fn(state) ->
  StatesApi.Repo.insert!(StatesApi.Estado.changeset(%StatesApi.Estado{}, %{
    sigla: state.sigla, nome: state.nome, sigla_regiao: state.regiao.sigla
  }))
end)

File.stream!(File.expand("../Correios/LOG_BAIRRO.TXT"))
|> Enum.each(fn(line) ->
  IO.puts(line)
end)
