defmodule StatesApi.Repo.Correios do
  ...
  defp insert_localidades do
    stream_file("LOG_LOCALIDADE.txt", fn(line) ->
      ...
      Repo.insert!(Localidade.new(%{
        ...
        tipo: tipo,
        abbr: to_utf8(abbr),
        ibge_municipio_id: String.strip(ibge_municipio_id)
      }))
    end)
  end

  defp insert_bairros do
    stream_file("LOG_BAIRRO.TXT", fn(line) ->
      [id, sigla_estado, localidade_id, nome, abbr] = String.split(line, "@")

      Repo.insert!(Bairro.new(%{
        id: id,
        sigla_estado: sigla_estado,
  ...
  defp stream_file(filename, f) do
    Path.expand("priv/repo/Correios/#{filename}")
    |> File.stream!()
    |> Enum.each(f)
  end
